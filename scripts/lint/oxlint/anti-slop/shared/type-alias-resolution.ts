...
// Cache only chains ending in keyword types: no substitutions or recursive
// branches can affect their result. Other alias forms retain upstream resolution.
const terminalAliasesByEnvironment = new WeakMap<
	TypeAliasEnvironment,
	WeakMap<ESTree.TSType, ESTree.TSType>
>();

function terminalAliasType(
	type: ESTree.TSType,
	environment: TypeAliasEnvironment,
): ESTree.TSType {
	let cache = terminalAliasesByEnvironment.get(environment);
	if (cache === undefined) {
		cache = new WeakMap();
		terminalAliasesByEnvironment.set(environment, cache);
	}
	const visited = new Set<ESTree.TSType>();
	let current = type;
	while (current.type === "TSTypeReference") {
		...
		if (visited.has(current) || current.typeArguments?.params.length) return type;
		visited.add(current);
		const name = typeReferenceName(current);
		const alias = name === null ? null : visibleTypeAlias(name, current, environment);
		if (alias === null || alias.typeParameters?.params.length) return type;
		current = alias.typeAnnotation;
	}
	if (!current.type.endsWith("Keyword")) return type;
	for (const reference of visited) cache.set(reference, current);
	return current;
}
...
export function resolvedTypeMatches(
	...
	const evaluate = (
		...
		if (substitutions.size === 0) current = terminalAliasType(current, environment);
